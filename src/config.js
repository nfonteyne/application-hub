...
module.exports = {
  ...
  pg: {
    ...
    database: process.env.PGDATABASE || 'application_hub',
    user: process.env.PGUSER || 'application_hub',
  ...
  adminGroupName: process.env.ADMIN_GROUP_NAME || 'hub-admins',
  ...
  // Garage (S3-compatible object storage) — used to store uploaded app
  // logos. Optional: without it configured, logo upload just returns a
  // clear error while everything else (URL/emoji icons) keeps working.
  garage: {
    endpoint: process.env.GARAGE_S3_ENDPOINT || null,
    region: process.env.GARAGE_REGION || 'garage',
    bucket: process.env.GARAGE_BUCKET || 'app-logos',
    accessKeyId: process.env.GARAGE_ACCESS_KEY_ID || null,
    secretAccessKey: process.env.GARAGE_SECRET_ACCESS_KEY || null,
    get configured() {
